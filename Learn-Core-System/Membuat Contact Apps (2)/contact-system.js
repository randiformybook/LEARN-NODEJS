...
const saveContacts = (nama, noHP, email) => {
  ...
  if (duplicate) {
    console.log(
      chalk.bgRed.bold(
        "Data yang Anda masukan,sudah terdaftar!, Mohon di periksa kembali"
      )
    );
    return false;
  }
  if (email) {
    if (!validator.isEmail(email)) {
      console.log(
        chalk.bgRed.bold(
          "Email yang Anda masukan tidak valid, Silahkan di periksa kembali"
        )
      );
      return false;
    }
  ...
  if (!validator.isMobilePhone(noHP, "id-ID")) {
    console.log(
      chalk.bgRed.bold(
        "No Handphone yang Anda masukan tidak valid, Silahkan di periksa kembali"
      )
    );
    return false;
  }
  ...
  console.log(
    chalk.bold.bgGreenBright(
      "Terimah Kasih, data sudah masuk di data contact kami"
    )
  );
